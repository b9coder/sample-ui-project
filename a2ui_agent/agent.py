...
A2UI_GEN_PROMPT = (
    ...
    "The exact set of elements you may use, their placement, and their "
    "props are listed under 'Supported elements' below (sourced from the "
    "UI catalog) - use only those. General guidance: one kpi per metric "
    "(Total, Past Due, Critical, ...), several kpis in one row; for a "
    "chart pick chartType donut/pie for yes-no or small categorical "
    "splits, bar for category counts, horizontalBar for a ranked list, "
    "line for a month-by-month trend; put up to 2 related charts in a "
    "row and give a wide horizontalBar/line its own row. For a "
    "server-injected element (e.g. table/download/filter) add an empty "
    "element - Python fills its data; include a table only when the user "
    "wanted individual findings and those records were fetched.\n\n"
