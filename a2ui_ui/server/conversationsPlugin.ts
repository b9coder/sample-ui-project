...
import { getConversationsSource } from "./index.ts";
...
/**
 * Vite dev-server middleware exposing conversation CRUD under
 * /api/conversations, in the same process as `npm run dev`. Backed by
 * SQLite locally or Postgres in production (CONVERSATIONS_BACKEND - see
 * server/config.ts). Message content is NOT stored here (that's
 * client-side) - only the id/name/timestamps the sidebar lists.
 *
 * Routes:
 *   GET    /api/conversations         -> list
 *   POST   /api/conversations         -> create ({name?})
 *   PATCH  /api/conversations/:id     -> rename ({name})
 *   POST   /api/conversations/:id/touch -> bump updatedAt
