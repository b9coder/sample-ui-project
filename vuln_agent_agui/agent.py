...
In addition to the vulnerability/risk/trend/entity-resolution MCP tools
(carried over unchanged from the original vuln_agent project), this agent
is also bound to an A2UI generation tool (`get_a2ui_tools`, from
ag-ui-langgraph) so it can dynamically render dashboards and on-screen
input forms via the open A2UI v0.9 protocol, on top of its existing
plain-text chat capability. See server.py for the AG-UI transport that
exposes this graph over SSE for an A2UI-aware frontend.
"""
...
SYSTEM_PROMPT = (
    ...
    "its own '---' divider.\n\n"
    "DYNAMIC UI (generate_a2ui) - you also have a generate_a2ui tool that "
    "renders an actual on-screen surface (cards, tables, KPI numbers, and "
    "real input widgets like text fields, checkboxes, sliders, and choice "
    "pickers) instead of plain chat text, using the A2UI protocol's basic "
    "component catalog (Text, Image, Row, Column, Card, List, Button, "
    "TextField, CheckBox, Slider, DateTimeInput, ChoicePicker).\n\n"
    "MANDATORY - call generate_a2ui (in the SAME turn, right after "
    "resolve_application/resolve_user returns) whenever entity resolution "
    "comes back status='ambiguous'. This is not optional and not a "
    "judgment call: render one Card per candidate inside a Column, each "
    "Card containing Text for the name/id and distinguishing details "
    "(business unit/environment, or department/email) plus a Button "
    "whose action submits that candidate's id - so the user clicks "
    "instead of retyping a choice. Still also list the candidates in your "
    "normal text reply as a fallback, but the generate_a2ui call itself "
    "is required every time status='ambiguous', with zero exceptions.\n\n"
    "OPTIONAL but encouraged - also use generate_a2ui (ADDITIONALLY "
    "alongside your normal text reply, not replacing the Criteria/"
    "Overview/Download structure) for:\n"
    "- A breakdown or ranking with several entries (severity, OS, "
    "business unit, owner, risk ranking): render it as a small dashboard "
    "of Cards/Rows showing each entry's name and value, which reads better "
    "than a long bullet list.\n"
    "- Whenever the user might want to refine their own search criteria "
    "(severity, date range, application, past-due/escalated/internet-"
    "facing flags): render a small filter form with the matching input "
    "widgets (ChoicePicker for severity, DateTimeInput for date ranges, "
    "CheckBox for boolean flags, TextField for free text) and a Button to "
    "submit it, instead of just describing what they could type.\n"
    "Skip the optional cases for simple single-number answers where a UI "
    "surface would be overkill - but never skip the mandatory ambiguous-"
...
async def build_agent():
    ...
    The returned CompiledStateGraph is used both by the custom REST /chat
    endpoint (server.py, carried over unchanged from vuln_agent) and by the
    AG-UI SSE endpoint (server.py's /agui route, wrapping this same graph
    with ag_ui_langgraph.LangGraphAgent) - one agent, two transports.
    """
