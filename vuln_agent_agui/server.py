...
Exposes the SAME compiled agent graph over two transports:
- POST /chat - the original custom REST endpoint (carried over unchanged
  from vuln_agent): plain JSON in/out, with this server's own reasoning-
  log/trusted-flag/chart extraction built on top.
- POST /agui - an AG-UI-protocol SSE endpoint (via ag_ui_langgraph), for
  an AG-UI/A2UI-aware frontend that wants generative-UI dashboards and
  on-screen input forms rendered from the agent's `generate_a2ui` tool
  calls, not just chat text.
...
async def lifespan(app: FastAPI):
    ...
    add_langgraph_fastapi_endpoint(
        ...
        LangGraphAgent(
            ...
            description=(
                "Security assistant over a vulnerability database, with "
                "risk ranking, remediation trend, and entity-resolution "
                "tools, plus dynamic A2UI dashboards/forms."
            ),
            graph=_agent,
        ),
        path="/agui",
